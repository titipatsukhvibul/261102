...
struct student{
	//Define struct student with four member (name ,id , gender, grade);
	string name;
	float id;
	string gender;
	float gpa;
...
int main(){
	...
	while(getline(student_file,textline)){
		...
		int firstCommaLocation = textline.find(",");
		s.name = textline.substr(0, firstCommaLocation);
		s.id = atof(textline.substr(firstCommaLocation + 1, 4).c_str());
		s.gender = textline.substr(firstCommaLocation + 6, 1);
		s.gpa = atof(textline.substr(firstCommaLocation + 8, 4).c_str());
